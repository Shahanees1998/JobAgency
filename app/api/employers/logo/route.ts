 ...
 * Upload company logo. Stores as base64 in DB.
  ...
  return withAuth(request, async (authenticatedReq: AuthenticatedRequest) => {
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        if (!file) {
          return NextResponse.json(
            { success: false, error: 'Logo image is required. Send "imageBase64" in JSON or "image"/"file" in FormData.' },
            { status: 400 }
          );
        }
        const fileObj = file instanceof File ? file : new File([file], 'logo.png', { type: file.type || 'image/png' });
      ...
      return NextResponse.json({ success: false, error: 'Failed to save company logo' }, { status: 500 });
    }
  });
}
