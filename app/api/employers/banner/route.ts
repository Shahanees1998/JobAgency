 ...
 * Upload company banner/cover image for current employer
 */
  ...
  return withAuth(request, async (authenticatedReq: AuthenticatedRequest) => {
    try {
      ...
      const employer = await prisma.employer.findUnique({
        where: { userId },
        select: { id: true, companyBannerPublicId: true },
      });
      ...
      if (!file) {
        return NextResponse.json(
          { success: false, error: 'Banner image is required. Send with field name "image".' },
          { status: 400 }
        );
      }
      ...
      else if (file instanceof Blob) fileToUpload = new File([file], 'banner.png', { type: file.type || 'image/png' });
      else {
        return NextResponse.json({ success: false, error: 'Invalid file format' }, { status: 400 });
      }

      const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
      const validation = validateFile(fileToUpload, { allowedTypes, maxSize: 8 * 1024 * 1024 });
      if (!validation.isValid) {
        return NextResponse.json({ success: false, error: validation.error }, { status: 400 });
      }

      // Upload to Cloudinary (banner/cover)
      const cloudinaryResult = await uploadToCloudinary(fileToUpload, {
        folder: `jobportal/employers/${employer.id}/banner`,
        resource_type: 'image',
        transformation: [
          { width: 1400, height: 500, crop: 'fill', gravity: 'center' },
      ...
      if (employer.companyBannerPublicId) {
        try {
          await deleteFromCloudinary(employer.companyBannerPublicId);
        } catch (e) {
      ...
      const updated = await prisma.employer.update({
        where: { userId },
        data: {
          companyBanner: cloudinaryResult.secure_url,
          companyBannerPublicId: cloudinaryResult.public_id,
        },
      ...
      return NextResponse.json({
        success: true,
        data: { companyBanner: updated.companyBanner },
        message: 'Company banner uploaded successfully',
      });
    } catch (error) {
      console.error('Employer banner upload error:', error);
      return NextResponse.json({ success: false, error: 'Failed to upload company banner' }, { status: 500 });
    }
  });
}
