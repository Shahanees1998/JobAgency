 ...
 * Upload profile image for current user. Stores as base64 in DB.
 * Accepts: application/json { imageBase64: "data:image/...;base64,..." }
 *          or multipart/form-data with "image" / "file" (converted to base64 and stored).
 */
  ...
  return withAuth(request, async (authenticatedReq: AuthenticatedRequest) => {
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        if (!file) {
          return NextResponse.json(
            { success: false, error: 'Image required. Send "imageBase64" in JSON or "image"/"file" in FormData.' },
            { status: 400 }
          );
        }
        const fileObj = file instanceof File ? file : new File([file], 'profile.png', { type: file.type || 'image/png' });
      ...
      const updatedUser = await prisma.user.update({
        ...
        data: {
          ...
          profileImagePublicId: null, // base64 stored in profileImage
        },
      ...
      return NextResponse.json(
        { success: false, error: 'Failed to save profile image' },
        { status: 500 }
      );
    }
  });
